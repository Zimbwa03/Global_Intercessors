...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/admin/statistics", async (req: Request, res: Response) => {
    try {
      ...
      // Get fasting registrations
      const { data: fastingRegistrations, error: fastingError } = await supabaseAdmin
      ...
      const totalSlots = prayerSlots?.length || 0;
      const activeSlots = prayerSlots?.filter(slot => slot.status === 'active').length || 0;
      const totalFastingRegistrations = fastingRegistrations?.length || 0;
      const totalUpdates = updates?.length || 0;
      const slotCoverage = totalSlots > 0 ? Math.round((activeSlots / 48) * 100) : 0; // 48 total possible slots

      // User activity breakdown
      const usersByRole = userProfiles?.reduce((acc: any, user) => {
      ...
      const statistics = {
        users: {
          total: totalUsers,
          byRole: usersByRole,
          active: userProfiles?.filter(user => user.is_active).length || 0,
        },
        prayerSlots: {
          total: totalSlots,
          active: activeSlots,
          coverage: slotCoverage,
          available: 48 - activeSlots,
        },
        fasting: {
          total: totalFastingRegistrations,
        },
        updates: {
          total: totalUpdates,
        },
